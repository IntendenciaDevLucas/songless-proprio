...
function App() {
  ...
  return <div className="app">
    ...
    {screen === 'game' && onlineRole === 'host' && hostWrongGuess && <div className="toast notice"><X size={18}/><span>Seu chute: <b>{hostWrongGuess}</b></span></div>}
    ...
    {screen === 'setup' && <main className="setup page">
      ...
      <section className="panel compact"><h3><span>4</span> Rodadas</h3><div className="round-options"><input type="number" min="1" max="100" value={roundCount} aria-label="Quantidade de rodadas" onChange={event => setRoundCount(Math.min(100, Math.max(1, Number(event.target.value) || 1)))}/><small>máximo 100</small></div></section>
